...
:- generate_domain(session(c1,s1), D1), length(D1, N1), format("Domain s1: ~w values~n", [N1]),
   generate_domain(session(c1,s7), D7), length(D7, N7), format("Domain s7: ~w values~n", [N7]),
   generate_domain(session(c2,s4), D4), length(D4, N4), format("Domain s4: ~w values~n", [N4]),
   halt.
