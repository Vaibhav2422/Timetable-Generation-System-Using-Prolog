...
:- (retrieve_resources(T,S,R,Sl,C) ->
    (length(T,NT), length(S,NS), length(R,NR), length(Sl,NSl), length(C,NC),
     format("T:~w S:~w R:~w Sl:~w C:~w~n",[NT,NS,NR,NSl,NC]),
     (validate_resources(T,S,R,Sl,C) -> format("validate OK~n") ; format("validate FAILED~n")),
     create_sessions(C,S,Sessions), length(Sessions,NSess),
     format("Sessions:~w~n",[NSess]),
     create_empty_timetable(R,Sl,Matrix),
     format("Matrix OK~n"),
     (solve_csp(Sessions,Matrix,_Timetable) -> format("CSP OK!~n") ; format("CSP FAILED~n"))
    )
   ;
    format("retrieve FAILED~n")
   ), halt.
